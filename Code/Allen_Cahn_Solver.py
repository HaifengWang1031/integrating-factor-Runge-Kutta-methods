...
class Allen_Cahn_fSolver_1D():
    def __init__(
        ...
        if self.bc == "periodic":
            ...
            self.U = u0[:-1][np.newaxis,:]
            self.fU = np.fft.fft(u0[:-1])[np.newaxis,:]
        elif self.bc == "neumann":
            raise
    ...
    def IFRK1(self,fUn,Un):
        return self.expM*(fUn + self.tau*np.fft.fft(self.f(Un)))
    ...
    def solve(self):
        ...
        while self.tn[-1] < self.T:
            ...
            self.U = np.concatenate([self.U,np.real(np.fft.ifft(fu))[np.newaxis,:]],axis=0) 
...
class Allen_Cahn_fSolver_2D():
    ...
    def fh_potential(self,theta = 0.8,theta_c = 1.6):
        ...
        f =lambda u: theta/2*np.log((1-u)/(1+u)) + theta_c*u
        # def f(u):
        #     print(theta/2*np.log((1-u)/(1+u)) + theta_c*u)
        #     return theta/2*np.log((1-u)/(1+u)) + theta_c*u
        return F,f
